...
const OngoingSellOrder = () => {
  ...
  useEffect(() => {
    if (ongoingSellOrderTransaction) {
      makeRows(ongoingSellOrderTransaction)
    } else {
      setRows(null)
    }
  }, [ongoingSellOrderTransaction])
  ...
  return (
    <>
      {rows && rows.length ? (
        ...
        >
          <Typography>
            No Ongoing Transactions Pending for Sell Order
          </Typography>
        </Box>
      )}
    </>
...
export default OngoingSellOrder
